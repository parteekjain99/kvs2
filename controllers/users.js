...
//       where: {
//         user_id: req.params.userId || userID.userId,
//       },
//       offset,
//       limit,
//     });
...
//     res.status(200).send({ data: user   , numberOfPost : post.length, numberOfFollower: user.followers_list.length,
//         numberOfFollowing: user.following_list.length});
//   } catch (error) {
...
exports.userDetails = async (req, res) => {
  try {
    ...
    res.status(200).json({
      code: 200, data: userData, block: block,
      posts: post.length,
    });
  } catch (error) {
...
exports.editProfile = async (req, res) => {
  try {
    ...
    if (!data) {
      res.status(400).send({ msg: "body  canot empty" })
    }



    if (data.profile_image || data.name || data.email || data.phone_no || data.dob || data.username || data.bio || data.city || data.state || data.country || data.pincode || data.complete_address || data.business_category || data.alternatePhoneno || data.businessname || data.website) {
      data.name = data.name
      ...
      data.complete_address = data.complete_address
...
exports.getaboutUs = async (req, res) => {
  try {
    ...
    let privacy_policy;
    if (!req.query.type) {
      terms_condition = await Models.Cms.findOne({ where: { type: "terms_condition" } });
      about_us = await Models.Cms.findOne({ where: { type: "about_us" } });
      privacy_policy = await Models.Cms.findOne({ where: { type: "privacy_policy" } });
    } else {
      result = await Models.Cms.findOne({
        where: {
          type: req.query.type,
        },
      });
...
exports.helpListing = async (req, res) => {
  try {
    console.log("result", req.query.type);
    const result = await Models.HelpType.findAll({});
    res.status(200).json({
      code: 200,
      data: result,
    });
...
exports.GetCitiesOfStateByStateCode = (req, res) => {
  ...
  var cities = require("country-state-city").City.getCitiesOfState(
    country,
    state
  );
  res.send({
